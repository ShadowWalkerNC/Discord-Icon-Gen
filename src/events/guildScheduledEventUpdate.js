        ...
        try {
            ...
            if (wasScheduled && isNowActive) {
                const embed = new EmbedBuilder()
                    ...
                    .setColor('#FF0000')
                    .setURL(`https://discord.com/events/${newEvent.guild.id}/${newEvent.id}`)
                    .setFooter({ text: 'Sigil • Event is starting now — join in!' })
                    .setTimestamp();
            ...
            if (wasActive && isNowComplete) {
                ...
                if (newEvent.scheduledStartAt) {
                    const diffMins = Math.round((Date.now() - newEvent.scheduledStartAt.getTime()) / 60_000);
